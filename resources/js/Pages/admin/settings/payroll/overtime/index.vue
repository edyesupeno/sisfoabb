...
const heads = ["Working Time", "Faktor Kali", "Deskripsi", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('settings.payroll.overtime.getdatalembur'), {
        params: {
            page: page,
...
const deleteHandle = async () => {
    axios.post(route('settings.payroll.overtime.deleteLembur'), { 'id': itemSelected.value.id }
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        <div class="flex flex-col md:flex-row md:-mr-px">
            ...
            <div class="grow">
                <!-- Panel Header -->
                <div class="border-b">
                    <h2 class="text-2xl text-slate-800 font-bold p-6">Setting Overtime Pay </h2>
                </div>
                    ...
                    <section>
                        <header class="block justify-between items-center sm:flex py-6">
                            ...
                            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                                ...
                                <VButton label="Add Setting" type="primary" @click="handleAddModalForm" class="mt-auto" />
                            </div>
                        </header>
                        ...
                        <VDataTable :heads="heads" :isLoading="isLoading" wrapperClass="!px-0">
                            ...
                            <tr v-for="(data, index) in query" :key="index" v-else>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.working_time }} Jam</td>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.persen }}</td>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.persen }} x 1/173 x {{ data.working_time
                                }} Jam
                                    x Normal Salary = Salary Overtime</td>
