...
const breadcrumb = [
    ...
    },
    {
        name: "Attendance",
        active: false,
        to: route('attendance.shift.index')
    },
    {
        name: "Shift",
        active: true,
...
const heads = ["Branch", "Name", "Shift Type", "Start Time", "End Time", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('attendance.shift.getdata'), {
        params: {
            page: page,
            search: searchFilter.value,
            filter_branch: filterBranchValue.value,
            sort_by_name: filter.value.sort_by_name,
            filter_normal_shift: filter.value.filter_normal_shift,
            filter_night_shift: filter.value.filter_night_shift
        }
    }).then((res) => {
...
const deleteHandle = async () => {
    axios.delete(route('attendance.shift.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
...
<template>
    ...
    <div class="mb-4 sm:mb-6 flex justify-between">
        <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Shift</h1>
                    ...
                    <section>
                        <header class="block justify-between items-center sm:flex py-6">
                            <h2 class="font-semibold text-slate-800">
                                All Shift <span class="text-slate-400 !font-medium ml">{{ pagination.total }}</span>
                            </h2>
                            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                                ...
                                <VButton label="Generate Shift" type="primary" @click="handleAddModalForm"
                                    class="mt-auto" />
                            </div>
                        </header>
                        ...
                        <VDataTable :heads="heads" :isLoading="isLoading" wrapperClass="!px-0">
                            ...
                            <tr v-for="(data, index) in query" :key="index" v-else>
                                ...
                                <td class="px-4 whitespace-nowrap h-16 capitalize"> {{ data.name }} </td>
                                <td class="px-4 whitespace-nowrap h-16">
                                    <VBadge :text="data.shift_type" :color="getTypeStatus(data.shift_type)" size="sm" />
                                </td>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.formated_start_time }} </td>
                                <td class="px-4 whitespace-nowrap h-16"> {{ data.formated_end_time }} </td>
