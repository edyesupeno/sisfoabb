...
const getData = debounce(async (page) => {
  axios
    .get(route("attendance.schedule.getdata"), {
      params: {
        page: page,
    ...
    .then((res) => {
      ...
      today.value = filter.value.valid_month
        ? new Date(filter.value.valid_month)
        : new Date();
...
const getShiftOptions = () => {
  axios
    .get(route("attendance.schedule.getshiftOptions"), {
      params: {
...
const getReport = debounce(async () => {
  axios
    .get(
      route("attendance.schedule.getreport", {
        search: searchFilter.value,
        filter_branch: filterBranchValue.value,
        valid_month: filter.value.valid_month,
        shift: filter.value.type,
...
const handleChangeSchedule = (user_id, date, type) => {
  const data = {
    date:
      dayjs(today.value).get("year") +
      "-" +
      (dayjs(today.value).get("month") + 1) +
      "-" +
      date,
    type: type,
  };
...
const updateSchedule = async (user_id, data) => {
  axios
    .post(route("attendance.schedule.update", { user_id: user_id }), data)
    .then((res) => {
...
const handleChangeImportExcel = (e) => {
  ...
  axios
    .post(route("attendance.schedule.scheduleimport"), formData)
    .then((res) => {
      if (res.data.meta.success == false) {
        notify(
          {
    ...
    <div class="grow">
      ...
      <section class="py-6 px-4">
        <header class="block justify-between items-center sm:flex py-6">
          <h2 class="font-semibold text-slate-800">All schedule</h2>
          <div
        ...
        >
          ...
          <tr v-for="(data, index) in query" :key="index" v-else>
            <td
              class="px-4 whitespace-nowrap bg-white h-16 w-56 font-bold fixed-left"
            >
              {{ data.name }}
            </td>
            <td
              v-for="(schedule, index2) in data.date"
              :key="index2"
              class="px-4 whitespace-nowrap h-16"
            >
              <VSelect
                placeholder=""
                ref="chooseSelectHandler"
                v-model="chooseValue[index].date[index2]"
                :options="shiftChoose"
                class="w-40 !z-0"
                :clearable="false"
                ...
                :disabled="
                  dayjs(
                    `${today.getFullYear()}-${today.getMonth() + 1}-${index2}`
    ...
    <section class="py-6 px-4">
      ...
      >
        ...
        <tr v-for="(data, index) in queryReport" :key="index" v-else>
          <td
            class="px-4 whitespace-nowrap bg-white w-56 font-bold capitalize fixed-left"
          >
            {{ index }}
          </td>
          <td
            v-for="(report, index2) in data"
            :key="index2"
            class="px-4 whitespace-nowrap h-16 bg-white w-56"
          >
            {{ report }}
          </td>
        </tr>
        <tr>
          <td
            class="px-4 whitespace-nowrap h-16 bg-white w-56 font-bold capitalize fixed-left"
          >
            Total
          </td>
          <td
            v-for="(totalReport, index3) in totalReport"
            :key="index3"
            class="px-4 whitespace-nowrap h-16 bg-white w-56 font-semibold"
          >
            {{ totalReport }}
          </td>
        </tr>
