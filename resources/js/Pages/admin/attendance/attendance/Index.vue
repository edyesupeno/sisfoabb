...
<template>
    ...
    <div class="bg-white shadow-lg rounded-md mb-8 p-4">
        <div class="mb-5 flex justify-between items-center">
            ...
            <div class="inline-flex space-x-2">
                ...
                <Datepicker v-model="filter.month" @update:modelValue="handleDate" month-picker :enableTimePicker="false"
    ...
    <div class="bg-white shadow-lg rounded-sm mb-8">
        ...
        <section class="py-6 px-4">
            ...
            <VDataTable :heads="attendanceListHeader" v-if="!attendanceListLoading" wrapperClass="!px-0"
                :freezeTable="true">
                ...
                <tr v-for="(data, index) in attendanceQuery" :key="index" v-else>
                    ...
                    <td class="px-4 whitespace-nowrap h-16" v-for="(attendanceData, attendanceIndex) in data.attendances"
                        :key="attendanceIndex">
                        ...
                        <div v-else class="p-1 border-2 rounded-lg border-slate-200"
                            ...
                            <VPresent v-else-if="attendanceData.status === 'present'"
                                @click="openModalForm = true, itemSelected = attendanceData" />
                            <VLate v-else-if="attendanceData.status === 'late'"
                                @click="openModalForm = true, itemSelected = attendanceData" />
                            <VAbsent v-else-if="attendanceData.status === 'absent'" />
                            <VClockoutEarly v-else-if="attendanceData.status === 'clockout_early'"
                                @click="openModalForm = true, itemSelected = attendanceData" />
        ...
        <section class="py-6 px-4">
            ...
            <VDataTable :heads="attendanceListHeader" v-if="!attendanceRecapLoading" wrapperClass="!px-0"
                ...
                <tr v-for="(data, index) in attendanceRecapQuery" :key="index" v-else>
                    <td class="px-4 whitespace-nowrap h-16 fixed-left capitalize"> {{ data.status.replace(/_/g, ' ') }}
                    </td>
                    <td class="px-4 whitespace-nowrap h-16" v-for="(recapData, recapIndex) in data.recaps"
                        :key="recapIndex">
