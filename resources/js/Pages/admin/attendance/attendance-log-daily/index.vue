...
const breadcrumb = [
  ...
  {
    name: "Attendance Log Daily",
    active: true,
    to: route("attendance.attendance-log-daily.index", {
...
// ])
const dateFilter = ref([
  dayjs(props.additional.filter_date).startOf("month").toISOString(),
  dayjs(props.additional.filter_date).endOf("month").toISOString(),
]);
...
const getData = debounce(async (page) => {
  axios
    .get(
      route("attendance.attendance-log-daily.getdata", {
        id: props.additional.employee.id,
      }),
      {
        params: {
...
const getAttendanceOverviewData = debounce(async (page) => {
  axios
    .get(
      route("attendance.attendance-log-daily.getdataoverview", {
        id: props.additional.employee.id,
      }),
      {
        params: {
...
const handleExport = () => {
  ...
  axios
    .get(
      route("attendance.attendance-log-daily.exportAttendanceLogDaily", {
        id: props.additional.employee.id,
        filter_date: dateFilter.value
      }),
      {
...
<template>
  ...
  <div class="mb-4 sm:mb-6 flex justify-start space-x-2 items-center">
    <Link
      :href="route('attendance.attendance-overview.index')"
    ...
    <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">
      Detail Log Daily - {{ additional.employee.user_detail.name }}
    </h1>
  </div>
  <!-- Content -->
    ...
    <div class="flex flex-col sm:flex-row md:-mr-px">
      ...
      <div class="grow">
        <div class="p-4">
            ...
            >
              ...
              <div class="text-slate-800">
                ...
                <div v-else>
                  ...
                  <div
                    class="font-medium text-sm !text-slate-500"
                    v-if="data.work_hours"
                  >
                    {{ data.work_hours }}
                  </div>
                </div>
          ...
          <section>
            <header
              class="block justify-between items-center sm:flex pt-8 pb-5"
            >
              ...
              <tr v-for="(data, index) in query" :key="index" v-else>
                ...
                <td class="px-4 whitespace-nowrap h-16 capitalize">
                  <VBadge
                    ...
                    :color="getTypeStatus(data.status)"
                    :icon="getIconStatus(data.status)"
                    size="sm"
                  />
                </td>
              </tr>
